    ...
    def watermark_processing(self, i, READING, INDEX, TEST_VID, directory, movement_detected, years):
        while READING:
            ...
            if directory.endswith('.MP4'): 
                ...
                if Excel_Automation.date_checker(self.date_in_watermark, month_holder, day_holder, year_holder, years) != True:
                    if watermark_values != '': 
                        # watermark_values != ''
                        # Width: 1920 
                        # Height: 1080
                ...
                if self.date_checker(self.date_in_watermark, month_holder, day_holder, year_holder, years) != True:
                    if watermark_values != '':
                        # watermark_values != ''
                        # Width: 1280 
                        # Height: 720
            ...
            try: 
                ...
                for camera in cameras: 
                     if camera in file_path: 
                         if self.video_compatability[j] == "Corrupt":
                             df = df.append({'TREETAG': camera, 'FILEPATH': file, 'FILENAME': file_name, 'Common': self.video_compatability[j]}, ignore_index=True)      
                         elif file_path.endswith('.MP4'):
                             if self.movement_detected_excel_input[j] == "":
                                df = df.append({'TREETAG': camera, 'FILEPATH': file, 'FILENAME': file_name, 'TEMPERATURE': self.temperature[j], 'YEAR': self.year[j], 'MONTH': self.month[j],
                            ...
                            if self.movement_detected_excel_input[j] == "":
                                df = df.append({'TREETAG': camera, 'FILEPATH': file, 'FILENAME': file_name, 'TEMPERATURE': '', 'YEAR': self.year[j], 'MONTH': self.month[j],
                                              'DAY': self.day[j], 'HH': self.hours[j], 'MM': self.minutes[j], 'SS': self.seconds[j], 'Common': self.movement_detected_excel_input[j]}, ignore_index=True)
                            elif self.movement_detected_excel_input[j] == "None":
                                df = df.append({'TREETAG': camera, 'FILEPATH': file, 'FILENAME': file_name, 'TEMPERATURE': '', 'YEAR': self.year[j], 'MONTH': self.month[j],
                                              'DAY': self.day[j], 'HH': self.hours[j], 'MM': self.minutes[j], 'SS': self.seconds[j], 'Common': self.movement_detected_excel_input[j], 'QUANTITY': 0}, ignore_index=True)    
